...
async function muteCommand(sock, chatId, senderId, durationInMinutes) {
    ...
    try {
        ...
        await sock.sendMessage(chatId, { text: `The group has been muted for ${durationInMinutes} minutes Hermex tech guy.` });

        setTimeout(async () => {
            await sock.groupSettingUpdate(chatId, 'not_announcement'); // Unmute after the duration
            await sock.sendMessage(chatId, { text: 'The group has been unmuted lets kill the vibe fans🥳.' });
